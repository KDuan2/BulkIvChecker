...
| `data/ranking-movesets.js` | Recommended movesets from `pvpoke/src/data/rankings/all/overall/rankings-{500,1500,2500,10000}.json` |
...
Last synced: 2026-06-21 against PvPoke `master` @ `821af1722af9eac3a10782a20e7c862bb740be3d`.

The data files (gamemaster, groups, meta-groups, rankings) are regenerated by
`tools/sync_pvpoke.py` — a stdlib-only script that fetches PvPoke's source from
GitHub at a pinned commit and writes all the wrapped JS/JSON outputs. It mirrors
upstream exactly, deleting any local `data/groups/*.json` that upstream no longer
ships. It does **not** touch `data/archived-cups.js` or the verbatim `js/pvpoke/`
engine.

To re-sync with a newer PvPoke release:

1. Copy the 7 verbatim source files from your local PvPoke clone into `js/pvpoke/` (manual; not handled by the script).
2. Run `python3 tools/sync_pvpoke.py --sha <commit>` (defaults to `master`) to regenerate `gamemaster_cache.json`, `data/gamemaster-data.js`, `data/groups/*.json`, `data/meta-groups.js`, `data/ranking-movesets.js`, and `data/ranking-scores.js`.
3. Update the "Last synced" line above with the date and commit you pinned to.
4. Run `compare_csv.py` against PvPoke's matrix to verify no regressions.
